---
title: Ignition Welder
revid: 161635
source: https://deepwoken.fandom.com/wiki/Ignition_Welder
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Items, Outfits, Expert Outfits, The Ignition Union]
---

# Ignition Welder

| Ignition Welder | |
|---|---|
| caption | Sturdy workwear designed for heavy duty labour. Just remember - personal protective equipment doesn't make you invincible! |
| tier | Expert |
| crafting_req | x5 Cloth, x5 Fiber, x5 Iron, x5 Erisore |
| price | 30 |
| stat_req | Power 10, 10 Fortitude |
| bankable | Yes |
| droppable | Yes |
| durability | 3000 Durability |
| resistances | +25% Physical Armor, +25% Elemental Armor |
| talents | Master Craftsman |
| obtainment | Saramaed HollowMiner's Landing |
| selling_price | 10 |
| weight | 1 |

**Ignition Welder** is an Expert [Outfit](Outfits.md) in _Deepwoken_.

## Statistics

### _Requirements_

This Outfit costs 300 and requires five (5) [Cloth](Cloth.md), five (5) [Fiber](Fiber.md), five (5) [Iron](Ores.md#iron), and five (5) [Erisore](Ores.md#erisore) to craft.

This Outfit additionally requires the player to be at least [Power](Power.md) 10 and have at least 10 [Fortitude](Attributes.md#ftd) to be crafted.

### _Armor Stats_

This Outfit has 3000 durability and grants +25% Physical resistance, +25% Elemental resistance.

It also gives the [Master Craftsman](Talents.md#artisan) Talent, allowing you to craft items that would normally require a workbench from your inventory.

## Trivia

-   This outfit is worn by [Ottilde](Ottilde.md) and [Merryweather](Merryweather.md).
