---
title: Etrean Wilds
revid: 227045
source: https://deepwoken.fandom.com/wiki/Etrean_Wilds
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Locations, Locations in the Etrean Luminant]
---

# Etrean Wilds

LocationL Etrean Wilds LocationR

The sounds of the forest comfort your sea-weary soul, if briefly.

  

**Etrean Wilds** is a location in _Deepwoken_. It is a sub-location of [Etris](Etris.md) found in [The Etrean Sea](The_Etrean_Sea.md).

## Description

Between the Etris Docks and Etris, there is a small forest, being the Etrean Wilds. It is relatively safe, but it is a popular location for players to hangout and spar with each other, so watch out for potential fights and potentially even ganks

## Points of Interest

### Navaen Nomad Camp

-   A small camp populated by [Navaen Nomads](Navaen_Hostage.md) found before entering Etris.
-   You can earn the [Way of Navae](Way_of_Navae.md) fist style by turning in a Navaen hostage to [Cheral](Cheral.md)
-   This is the location of the first part of the [Zana](Zana.md) [quest](<Poser's_Ring.md>).
-   You can buy the following here:
    -   [Navaen Nomad Robes](Navaen_Nomad_Robes.md) for 45.
    -   [Lumber Axe](Lumber_Axe.md) for 3.
    -   [Flint](Flint.md) for 3.

### Ruined House

-   A small ruined house found below Etris that has importance with the [Kelsius](Kelsius.md) Book quest.
    -   When doing the "Lost Books" Quest for the first time, the player guided to the house. [Khan](<Khan_(NPC).md>) can be found here.

## Maps

A map can be found at [Maps#Surface](Maps.md#surface)

## Trivia

-   The music for this area is [Solivagant](Deepwoken_OST.md#solivagant) by Naktigonis
-   This area was previously called the "Etrean Woods" during testing.
