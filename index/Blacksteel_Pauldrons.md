---
title: Blacksteel Pauldrons
revid: 60744
source: https://deepwoken.fandom.com/wiki/Blacksteel_Pauldrons
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Outfits, Arms, Article stubs, Character]
---

# Blacksteel Pauldrons

| Blacksteel Pauldrons | |
|---|---|
| type | Arms |
| rarity | Common |
| selling price | Unknown |
| requirements | None |
| weight | 2 |
| innate stats | +Grants Random placeholder stats |
| innate talent | None |
| bankable | Yes |
| droppable | Yes |
| enchantable | Yes |
| pass down | Yes |
| obtainment | Erisia, Blacksteel Pirates |

## Description

The Blacksteel Pauldrons are an equipment item in the arms category. The stats and pips gained from equipping them have not been documented. They are commonly found in chests from [Erisia](Erisia.md), and can be dropped from [Blacksteel Pirates.](Blacksteel_Pirate.md)

## Trivia

-   The Blacksteel Pauldrons are part of the "Blacksteel" set; with the only other piece being the [Blacksteel Helm.](Blacksteel_Helm.md)
