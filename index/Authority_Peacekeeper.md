---
title: Authority Peacekeeper
revid: 30222
source: https://deepwoken.fandom.com/wiki/Authority_Peacekeeper
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
---

# Authority Peacekeeper

Template:EnemyInfobox1.0

## Description

The Authority Prison Guard is a type of enemy NPC found patrolling in [Fort Merit](Fort_Merit.md). They may look intimidating but in actuality they are surprising weak compared to an [Authority Footsoldier](Authority_Officer.md).

## Moveset

Their arsenal is composed of a [Rifle Spear](Weapons.md) and the [Flamecharm](Flamecharm.md) attunement:

### Moves:

| Name | Desciption | Parryable? | Blockable? | Danger |
| --- | --- | --- | --- | --- |
| Assault | Firmly thrusts spear at opponent and continuing to combo them until knocked. | ◯ | ◯ | (★☆☆) |
| Shoot | Pulls the trigger on the spear and fires a bullet at opponent. | ◯ | ◯ | (★★☆) |
| Fire Blade | Slicing blades of flame. | ◯ | ◯ | (★★☆) |

## Tips

-   Fighting them head-on is probably the best way of attack, considering they are pretty "dumb" in combat.
-   Try not to let them alert the others by calling "Hey, Over Here!".
-   ~If their numbers have increased by a lot, it's best to keep backing off and to knock all of them one by one.~
-   It's actually better to run and heal instead of trying to pick them off one by one due to lag and other server-side issues. Especially if your not good at fighting multiple opponents at once.
-   Avoid grabbing the attention of an Authority Warden into the fight, as he will pose a great threat.
