---
title: The Depths (Vow of Iron)
revid: 182204
source: https://deepwoken.fandom.com/wiki/The_Depths_(Vow_of_Iron)
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
---

# The Depths (Vow of Iron)

The depths of V.O.I. differ from Pathfinder, there's more danger there than Pathfinder depths

## Differences from Pathfinder

-   You spawn in the Cathedral of the Interstice.
-   No [Deep shrines](Deep_Shrines.md) or [Yuns](<Shrine_of_Yun'Shul,_Keeper_of_Hearts.md>)[hul](<Shrine_of_Yun'Shul,_Keeper_of_Hearts.md>).
-   There are no jobs on the job boards.
-   Diluvian mechanism no longer works.
-   You no longer receive health packs from enemies.

<table><caption>Vow Of Iron Depths Trial (Please update if you find more enemies)</caption>
<tbody>
<tr>
  <th>Power</th>
  <th>Enemies</th>
</tr>
<tr>
  <td>1-5</td>
  <td><a href="Bone_Keeper.md">Bone Keeper</a>, or Lightning <a href="Nautilodaunt_Officiate.md">Nautilodaunt Officiate</a></td>
</tr>
<tr>
  <td>6-10</td>
  <td>???</td>
</tr>
</tbody>
</table>
