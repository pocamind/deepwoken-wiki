---
title: Thundercall
revid: 3469
source: https://deepwoken.fandom.com/wiki/Thundercall
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Magic]
---

# Thundercall

> "lighting zap haha" - Former Description

-   **Description**

Thundercall icon

An element utilizing lightning based attacks.

-   **Training**

Can be trained by using [Self-Conducting Loop](https://deepwoken.fandom.com/wiki/Self-Conducting_Loop?so=search)

-   **Spells**

Lightning Blade- A dash foward that unsheathes your weapon and cuts twice with a lightining infused blade.

Lightning Clones- A spell that summons two clones of the player and attack whoever has either: Aggro on the player, or whoever the player attacked within the last few seconds of casting.

Jolt grab- A dash with a charged up fist of lightning that grabs the opponent, lifts them up into the air, and slams them down the lightning, (similar to thunder spear crash from rogue, but a much smaller scale)

Grand Javelin- Throws a spear that ragdolls enemies and flings them into the air

Lighning stream- Grapples onto a enemy and pulls them in (if you hit them) (also stuns them for awhile you may be able to get in an m1 or two before they start parrying/blocking)

Lightning beam- a beam of lightning that does damage (does not fling)

Lightning Impact- You fly up in the air and slam down

**Trivia and Notes**

_A Thundercall infused sword_

Mantras can be upgraded at a guy in the Summer Isles (the place you buy guns at) for 100 notes. make sure your thundercall level is high enough though.

Thundercall moves are very ether demanding, so it's best to use a character rich in ether.
