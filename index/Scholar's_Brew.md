---
title: "Scholar's Brew"
revid: 216257
source: https://deepwoken.fandom.com/wiki/Scholar's_Brew
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Food]
---

# Scholar's Brew

| Scholar's Brew | |
|---|---|
| weight | 2 |
| stack | x10 |
| type | Food |
| food type | Generic |
| stomach | 3% |
| water | 25% |
| buff | Ether (Increased Ether regeneration) |
| buff strength | 15% |
| bankable | Yes |
| droppable | Yes |
| pass down | No |
| desc | Food for thought? |
| obtainment | Crafting |

## Description

**Scholar's Brew** is a craftable [Food](Food.md) item.

It is a type of **Generic** food.

### Effect

When consumed, it replenishes 3% Stomach and 25% Water.  
  
Additionally, it grants the Ether buff, affecting 15% of the Stomach bar. The buff increases Ether regeneration by 20%.

It has a unique effect where it grants EXP for all [Attunements](Attunements.md) you currently have, useful for training Attunements if you haven't yet obtained a [Mantra](Mantras.md) for that Attunement. This effect only works if said Attunement is below 50 points. Past that point, it does not grant any EXP.

## Crafting

### Recipe

-   1 of any [Mantra Modifier](Mantra_Modifiers.md) and 1 of any [Mushroom](Ingredients.md#mushrooms) (excluding [Bluecaps](Bluecap.md)) at a [Campfire](Campfire_Pit.md) [CraftingCampfire](Campfire_Pit.md).
-   1 [Bluecap](Bluecap.md) and 1 of any [Mushroom](Ingredients.md#mushrooms) at a [Campfire](Campfire_Pit.md) [CraftingCampfire](Campfire_Pit.md).

## Trivia

-   The Scholar's Brew requires Mantra Modifiers to craft, **NOT** Attunement-based ingredients such as [Dying Embers](Dying_Embers.md).
