---
title: Guild Banners
revid: 204136
source: https://deepwoken.fandom.com/wiki/Guild_Banners
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Guilds, Mechanics]
---

# Guild Banners

| Guild Banners | |
|---|---|
| usage | Generates Notes for a Guild's treasury when captured |

## Description

**Guild Banners** are claimable landmarks that rewards 4-21 notes every 30 seconds to its controlling [Guild's](Guilds.md) Treasury, depending on how many players from other guilds are in the server.

When capturing a Guild Banner, it will bear the emblem of the player's guild and gradually gain health at a fixed rate. The banner's health gain can be sped up if a player is within its radius.

Standing near a Guild Banner controlled by an opposing guild will gradually decay its health, at which point the banner is then captured by the guild the player is in. Afterward, staying near a controlled banner will revert capture progress. Guild Banners passively revert capture progress to their owner as long that they aren't contested by enemies.

The visual marker above the Guild Banner will change depending on its current status:

-   White - Uncontested/Gaining health.
-   Red - Contested by an opposing guild.
-   Green - Controlling guild is within a banner's radius.
-   Orange - Contested by multiple guilds.

## Locations

If you are lost on where to find any of these, check out the Etrean and Eastern Luminant [Maps](Maps.md).

### [Etrean Luminant](Luminants.md)

#### [Erisia](Erisia.md)

1.  One right outside of [Erisor's Church](<Erisor's_Church.md>).
2.  One on a small island near [Lower Erisia](Lower_Erisia.md).
3.  One right outside [The Birdcage](The_Birdcage.md).
4.  One on a mountain near The Birdcage.
5.  One atop a large tower at [Towerstruck Lands](Towerstruck_Lands.md).

#### [Minityrsa](Minityrsa.md)

1.  One in the tower ruins above [Outpost Antumbra](Outpost_Antumbra.md).
2.  One right next to a statue at Statue Isle ([The Etrean Sea](The_Etrean_Sea.md)).

### [Eastern Luminant](Luminants.md)

#### [Aratel Sea](The_Aratel_Sea.md)

1.  One on a small island near [Miner's Landing](<Miner's_Landing.md>).
2.  One on the island with [The Entropy Catalyst](The_Entropy_Catalyst.md) and [War Mode](War_Mode.md) Bell.
3.  One near the highway, next to the [Linkstrider](Oath%253A_Linkstrider.md) meteor.

-   One on a small island across from the [Summer Isle](Summer_Isle.md) Guildhall.

#### [Miner's Landing](<Miner's_Landing.md>)

1.  One on the bridge to [Starfield Veldt](Starfield_Veldt.md), nearby [Adar](Adar,_Master_of_Metal.md).

#### [Songseeker Wilds](Songseeker_Wilds.md)

1.  One by the bridge across from [Derelict Highchurch](Derelict_Highchurch.md).
2.  One in front of the large windmill.
3.  One next to the [Linkstrider](Oath%253A_Linkstrider.md) meteor.

#### [The Starswept Valley](The_Starswept_Valley.md)

1.  One in the ruins near the Legion Camp on the way to the [Deep Widow](Deep_Widow.md).
