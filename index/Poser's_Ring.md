---
title: "Poser's Ring"
revid: 227503
source: https://deepwoken.fandom.com/wiki/Poser's_Ring
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Equipment, Rings]
---

# Poser's Ring

| Poser's Ring | |
|---|---|
| type | Rings |
| rarity | Common |
| selling price | N/A |
| requirements | N/A |
| weight | 0.1 |
| innate stats | +1 Rare Pip |
| bankable | No |
| droppable | No |
| enchantable | No |
| pass down | No |
| obtainment | Zana |
| description | Damage done with a Katana isn't received until the user sheathes their weapon. |

## Description

The Poser's Ring is an [Equipment](Equipment.md) item under the "Rings" category. It can be obtained by completing [Zana's](Zana.md) quest.

While using a [Katana](Katana.md), [Alloyed Katana](Katana.md), [Palace Tachi](Palace_Tachi.md), or [Shattered Katana](Shattered_Katana.md), the Poser's Ring makes it so that your Katana's attacks and critical deal 0 damage until the user "Sheathes" their Katana. When this happens, all damage dealt by the Katana will be dealt instantly. See [Poser's Ring (mechanic)](Stored_Damage_Mechanic.md) for more info.

## Trivia

-   Damage from the Poser's Ring sheathe cannot exceed 70% of a Player's maximum health.
    -   **All non-player enemies are not affected by this cap**, meaning with enough M1s, you are capable of depleting even boss health bars from 100% to 0% in one sheathe.
-   Effects that are scaled off of your weapon's scaled damage, like [Bleed](Status_Effects.md#bleed) or Chip damage, still work on Poser's Ring, dealing their effects on weapon hit, and not on sheathe.
-   The sheathe damage cannot be directly buffed, although you can buff it by applying buffs to your melee attacks before you sheathe.
-   It is possible to obtain and equip this ring at Power 1, if no stat points are distributed at the character creation menu, and are instead all invested into Medium Weapon.
-   Shares the same model as the [Diver's Ring](<Diver's_Ring.md>).
