---
title: Shadowcast
revid: 6023
source: https://deepwoken.fandom.com/wiki/Shadowcast
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
---

# Shadowcast

## Description

Shadowcast is an element that the player is required to unlock and can not start with. In order to unlock shadowcast, the player must bring five [umbral obsidian](Umbral_Obsidian.md) to the Depths and talk to the respective trainer. Upon drinking the Nightblood item, the Shadowcaster talent will be given and the shadowcast element will be unlocked permanently. (Note: After shadowcaster shows up in stats menu you can wipe to get it to show in your character creation.)

Shadowcast is unique due to the fact that it drains ether from those that are hit by shadowcast spells and returns some ether to the caster after succesfuly landing a spell. Multiple shadowcast based talent cards revolve around this mechanic.

## Training

Shadowcast is an element thats belongs to Shadow, to train this element, you need to buy Pure Heart.

## Mantras

**COMBAT**

| Image/Gif | Spell | Card Description | Effect |
| --- | --- | --- | --- |
|  | Dark Blade | Slicing blades of dark energy | A dash foward that unsheathes your weapon and cuts three times with a shadow infused slice. |
|  | Shadow Gun | An awful blast of dark energy | You fire a melee-ranged shot at your mouse cursor. |
|  | Clutching Shadow | Power lurks within. | Chokes target enemy and slams them down. |

It is unknown if 3 star combat mantras exist or not

**SUPPORT**

<table>
<tbody>
<tr>
  <td></td>
  <td>Shadow Meteor (☆☆)</td>
  <td>Hail down meteors of darkness.</td>
  <td>Three meteors fall depending on where you aim your cursor. They may get stuck if it would hit a surface before where your cursor is pointed towards.</td>
</tr>
</tbody>
</table>

**MOBILITY**

<table>
<tbody>
<tr>
  <td></td>
  <td>Shadow Assault<p>(☆☆)</p></td>
  <td>A rush of darkness.</td>
  <td>You dash forwards, hitting anybody in the way.</td>
</tr>
</tbody>
</table>
