---
title: Anklets of Alsin
revid: 83839
source: https://deepwoken.fandom.com/wiki/Anklets_of_Alsin
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Weapons, Fists]
---

# Anklets of Alsin

| Anklets of Alsin | |
|---|---|
| type | Kick Style (Jus Karita) |
| requirements | [REQUIRES A KICK STYLE] |
| damage | 13.5 |
| scaling | 9 |
| range | 7 |
| swing speed | 0.9x |
| endlag | 0s |
| penetration | 20% |
| chip damage | 10% |
| special effect | None |
| selling price | 30 Notes |
| weight | 3 |
| bankable | Yes |
| droppable | Yes |
| enchantable | Yes |
| pass down | Yes |
| obtainment | Duke Erisia chest, Chaser chest, Fishing chest, Deep Widow chest, Primadon chest,The Depths and Fort Merit hostage quest chest, Dread Serpent direct drop, Voidwalker bounty chest |
| description | [REQUIRES A KICK STYLE] |

## Description

The Anklets of Alsin are a light weapon that can be found in all depths chests, [Chaser's](Chaser,_Scholar_of_the_Crimson_Contract.md) chest, [Fort Merit](Fort_Merit.md) chests, [Deep Widow's](Deep_Widow.md) chest, Primadon chest, Dread Serpent (direct drop), Fishing chest, [Duke's](Duke_Ishamon_Erisia.md) chest, and Voidwalker bounty chests. The only requirement is a Kick Style. As of now, the only Kick Style is [Jus Karita](Jus_Karita.md).

## Notes

-   This is the first weapon released for the [Jus Karita](Jus_Karita.md) fighting style.
-   This is also the first weapon to be modelled on the player's legs instead of hands and or arms.

## Benefits

Serves as a stat-stick to give players:

-   Access to [Stars](Weapons.md).
-   Access to [Enchants](Enchantments.md).
-   +10% Chip Damage.
