---
title: Loading Screen
revid: 225653
source: https://deepwoken.fandom.com/wiki/Loading_Screen
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [In progress without reason, InProgressPages]
notices:
  - This page is currently being worked on. Some things may be incomplete.
---

# Loading Screen

The **Loading Screen** comes up every time a player teleports to a new server, including when starting up the game.

## Description

During the loading processes, the resources are rendered in so the player can interact with them. Loading occurs when any of these events are triggered:

-   Starting and exiting the game
-   Teleporting to a location found within a different server.
    -   Entering or exiting a [Dungeon](Dungeons.md)

### Area Flavor Texts

-   Accessing Memories of the past... - Defeating a core [Mementos](Mementos.md) [Boss](Bosses.md).
-   Calling back to Memories departed... - Entering the [Tutorial](Tutorial.md).
-   Descending beyond comprehension... - Dying in a [Dungeon](Dungeons.md) and being sent to [Fragments of Self](Fragments_of_Self.md) or [Fragments of Else](Fragments_of_Else.md).
-   Heeding Conflict's call... - Entering the [Chime of Conflict](Chime_of_Conflict.md).
-   Learning some new tricks... - Entering [Training Mode](Training_Mode.md).
-   Returning to Menu... - Entering the Main Menu.
-   Sinking deep into the Depths... - Entering the [First Layer](First_Layer.md).
-   The City awaits... - Entering [New Kyrsa](Second_Layer/New_Kyrsa.md).
-   The Drowned City of Celtor comes into view once more. - Entering the [First Layer](First_Layer.md).
-   The Eternal Gale coils around you... - Entering [The Ethironal Shrine](Second_Layer/The_Ethironal_Shrine.md).
-   Travelling... - Entering the Main Menu.
-   Venturing forth into a Dungeon. - Entering a [Dungeon](Dungeons.md).

## Tips

### Combat

#### Attunements

-   [Ironsing](Ironsing.md), the Attunement that enables control over metals, is an art once thought lost to the Tides. However, rumors from Miner's Landing speak of a genius Union researcher seeking to revive it...
-   [Shadowcast](Shadowcast.md), the Attunement that enables control over shadows, is a taboo art often shunned as the magic of cultists. However, those willing to see past superstition may find willing mentors in secluded places...
-   [Weather](Environmental_Hazards.md) can affect elemental damage. For example, [Lightning](Thundercall.md) is stronger in Rain, and [Frost](Frostdraw.md) is stronger in Snow.
-   Wielding any [Mantras](Mantras.md) of an element grants you [Cantrips](Elemental_Cantrips.md) for that element. Cantrips are minor feats available to wielders of an element, such as lighting fires or purifying drinking water.

#### Chime of Conflict

-   Those who wish to hone their skills in combat may find what they seek by speaking with [Yamaketzal](Yamaketzal.md), a mysterious [Capra](Capra.md) who hangs around the [Songseeker Wilds](Songseeker_Wilds.md) in the [Eastern Luminant](Luminants.md). With his help, warriors will find their mettle tested against many other warriors from distant lands.

#### Posture

-   Landing a Critical Attack will clear 30% of your Posture so long as it isn't parried or dodged.
-   Your Posture Bar will flash red when you attempt to block while your block is 'Fragile'. You should try to parry, dodge, or vent while in this state!
-   Your Posture Bar will flash white when your Parry comes off cooldown. Timing is everything!

#### Weapons

-   Heavy Weapon Users can wield their weapons one-handed once they have 40 HVY.
-   Heavy weapons have an innate +10% damage to Armor on larger foes.
-   Heavy weapon users that parry an attack are granted brief hyperarmor frames towards the end of their next Basic Attack.

### Armor

-   Armor provided by Equipment is tied to your Outfit's Armor Durability. If it runs out, your Equipment won't provide its Armor Resistances either.
-   If your Armor bar hits zero none of your Armor Resistances will work! This isn't just limited to your Outfit, but also to your [Equipment](Equipment.md).

#### Equipment

-   [Equipment](Equipment.md) pieces usually come with 'pips', with each star of Quality adding another pip. Pips are randomly generated per equipment piece, so every drop you get may well be different!
-   Each star of Quality on a piece of Equipment grants +1 HP.
-   Soulbound Equipment and Enchanted Equipment cannot be lost on death, but will be lost on character loss unless you pass them down at the [Hippocampal Pool](Hippocampal_Pool.md).

#### Outfits

-   Damage resisted by your Armor depletes an equivalent amount of your Armor Durability. Once depleted fully, your Armor will be broken an no longer provide any resistances. You can repair Armor at a [Smith](Blacksmith.md), or by using a [Repair Kit](Repair_Kit.md).
-   PEN is short for Armor Penetration and represents how much of your enemy's Armor Resistances your attacks ignore. For example, if you have 50% PEN on your Physical attack and your opponent has 50% Physical Armor, they'll only resist 25% of the damage.
-   Your Armor Resistances from different sources are multiplied together, _not_ added. This means 50% Physical Armor from your Outfit and 50% Physical Armor from your [Equipment](Equipment.md) will resist 75% of Physical Damage, not 100%!  
      
    You can see how much Armor Resistance you have total in your Journal.

### Factions

-   Be careful swinging around a blade in a faction's territory - you might attract the attention of the guards!
-   It's rumored that the deadly [Knives of Eylis](The_Knives_of_Eylis.md) are [looking for new recruits](Voidwalker.md)...
-   Sticking around long enough may attract the attention of the [Knives of Eylis](The_Knives_of_Eylis.md), a feared group of mercenaries with the uncanny ability to phase in and out of reality. Fending one off or falling to them will prevent more from hunting you for a while.

### Locations

-   Expeditions into the frozen wastes of [Minityrsa](Minityrsa.md) suggest that [the Ministry](The_Ministry.md) may have unearthed [some form of ritual site](Outpost_Antumbra.md). Those who seek [a challenge](Trial_of_One.md) might seek it out.
-   Rumors speak of the [Temple of Hearts](Temple_of_Hearts.md), [a mysterious shrine](<Shrine_of_Yun'Shul,_Keeper_of_Hearts.md>) that many seek out to right their wrongs and soothe their regrets...
-   Those that Drown and wish to escape must seek out the [Cathedral of the Interstice](Cathedral_of_the_Interstice.md) where a trial awaits them...
-   [Voidzones](Voidzones.md) are areas that have slipped into [the void](Voidsea.md), bringing them closer to [the Depths](The_Depths.md). Dying there will send you straight down to the Depths, but all forms of experience are doubled in their harsh conditions.

### Mechanics

-   Before losing your character forever, you can pass down 2 pieces of equipment using the [Hippocampal Pool](Hippocampal_Pool.md). This number can be increased by upgrading your passdown capacity using [Echoes](Echoes.md).
-   Find something interesting? Take it to the [Antiquarian](Antiquarian.md) for appraisal!
-   Rumors say that gesturing to a certain memorial can attract [a dangerous disease](Destroyman_III.md)...
-   The Natural Armor provided by [Chitin](Talents.md#chitin) and [Exoskeleton](Talents.md#exoskeleton) can be replenished by resting at [campfires](Campfire_Pit.md).
-   Those that come close the death in [the Depths](The_Depths.md) may be driven to push themselves to the limit and unlock [a latent potential within themselves](Resonance.md)...
-   Want to take someone out of combat after you've hit them? Click the crossed swords ar \[_[sic](https://en.wikipedia.org/wiki/sic)_\] the top of your screen for options.

### Oaths

-   [Arcwarders](Oath%253A_Arcwarder.md) are bulwarks of fortitude that wield an adaptive suit of armor crafted by [the Ignition Union](The_Ignition_Union.md). Seekers of this [Oath](Oaths.md) should seek out the brave Union Delvers carving their way through the Depths below...
-   [Blindseers](Oath%253A_Blindseer.md) are bastions of hope in a world ravaged by nightmares. By willingly blinding themselves, they soothe the minds of others and fight back the horrors of the Deep. Seekers of this [Oath](Oaths.md) should look to the [ruins of old](Cathedral_of_the_Interstice.md)...
-   [Chainwardens](Oath%253A_Chainwarden.md) are resilient enforcers that utilise their chains to bind and restrain their foes. Those who seek this [Oath](Oaths.md) must seek out [those that mete out punishment](Vice_Warden_Morredh.md) and enforce their own sense of justice...
-   [Contractors](Oath%253A_Contractor.md) are skilled manipulators that utilise puppet strings to control their foes. Adherents of [the Ministry](The_Ministry.md)'s creed, those that seek this [Oath](Oaths.md) will need to swear themselves to [the Ministry](The_Ministry.md) and their shadowy motives...
-   [Dawnwalkers](Oath%253A_Dawnwalker.md) are resplendent beacons of light that dedicate themselves to the eradication of [the Depths](The_Depths.md). A product of the genius mind of [Klaris Llfiend](Klaris_Llfiend,_Mother_of_Lights.md), those that seek this [Oath](Oaths.md) will need to prove themselves to her...
-   [Fadetrimmers](Oath%253A_Fadetrimmer.md) are [Barbers](Barber.md). If you seek this [Oath](Oaths.md), ask nicely and they might lend you their scissors.
-   [Jetstrikers](Oath%253A_Jetstriker.md) are flowing and agile sprinters who drift wherever the winds take them. Lovers of competition, seekers of this [Oath](Oaths.md) should find [a Jetstriker in need of a rival](Alirian.md)...
-   [Saltchemists](Oath%253A_Saltchemist.md) are geniuses of [chemistry](Alchemy.md) who sling all forms of concoctions at their foes and allies alike. Those who seek this [Oath](Oaths.md) may be in need of a research partner and a laboratory to conduct their studies...
-   [Silenthearts](Oath%253A_Silentheart.md) are fierce avengers who reject the Words of the Song. Feared by many, those who seek its vengeance must find the [Heart of Dread](The_Dreadstar.md) at the [furthest reaches of Lumen](Voidsea.md)...
-   [Starkindreds](Oath%253A_Starkindred.md) are sworn adherents to their faith that adopt the shapeshifting capabilities of their [Gods](Celestial.md) to adapt to any situation in battle. Those who seek this Oath must prove themselves as dedicated and pious servants of [the Starkindred Faith](The_Starkindred_Church.md)...
-   [Visionshapers](Oath%253A_Visionshaper.md) are duplicitous and cunning illusionists that bend and warp reality to fool others. Those who seek this [Oath](Oaths.md) must seek [those that adopt the mannerisms](Surge.md) of the [Owls](Deep_Owl.md), those that inspired the Visionshaper creed...

### Roblox Chat

-   Preceding a chat message with '!' makes it a Shout and visible to all of your allies in a server.

### Vows

-   You can make a Vow of Allegiance with another player by asking _'wanna make a vow of allegiance?'_. This will cause them to be considered an ally by your abilities.

### Official Community Server

-   Died unfairly to a bug or exploiter? If you have a recording of the death, you can create a restore ticket in the Community Server. Please consult the restore guidelines beforehand for a comprehensive list of what is and is not restorable.

### Miscellaneous

-   Ever feeling bored? Maybe try out a challenge! How about trying to go for all echo triumphs? Or doing that with all echo modifiers on? Or defeating a boss with minimal damage taken?

* * *

### Combat

-   Hitting unconscious players will drain their blood, eventually bleeding them out.
-   Charmed enemies deal reduced damage to those that applied the status.
-   Accidentally disconnected or crashed while in danger? So long as you were above 60% HP at the moment your connection cut out, you should not have lost a life. This does not apply to PvP.
-   Killing certain tough monsters for the first time rewards you with 1 to 2 Knowledge.
-   Pressing Feint/Cancel while in ragdolled knockback will let you 'tech' the ragdoll state, speeding up your recovery.
-   Wielding any Mantras of an element grans you Cantrips for that element. Cantrips are minor feats available to wielders of an element, such as lighting fires or purifying drinking water.

### Mechanics

-   Ever afraid of accidentally selling an item you want to keep? Right click on it and lock it!
-   After standing still in one spot for 15 seconds while injured, you'll start to Recuperate, catching your breath and accelerating your natural healing.
-   Certain spots around the world are better for fishing. If you find water that splashes and ripples, you've found an especially good spot!
-   Damage Multipliers have a soft limit at +25%, where the effectiveness of damage multipliers is halved. The hard limit is at +50% damage effectiveness, achieved by having a total damage multiplier of +100%.
-   Monsters do not block and therefore do not suffer posture damage. They do, however, have Armor just like players do. Damage resisted by their Armor depletes their Durability, and once broken the monster will be stunned for several seconds as their Armor recovers.
-   In trouble with the law? Some less scrupulous individuals may be willing to take a bribe to fix your reputation...
-   Parrying a Mantra will absorb a portion of the Ether used to cast it.
-   Different Origins allow for different styles of progression, spicing up your playthrough!
-   Some interfaces can be right-clicked for more options. Try right-clicking an item in your Backpack!
-   Map looking empty? Your map will fill out as you explore the world - who knows what you might find in uncharted territory?
-   Staggered monsters will lose all their resistances temporarily, taking more damage from incoming attacks.
-   While not in a Dungeon, chaining horizontal mobility moves in succession lessens their effectiveness.
-   Dashing backwards in midair performs a flip that greatly reduces your velocity, letting you recover from knockback.
-   The depth of water you fish at affects what you'll catch. Some fish are best caught at the shore, and others far out on a boat!
-   Most fish are either freshwater or saltwater fish - though some are euryhaline, meaning they can be found in either!

### Oaths

-   An Oath is an extension of the Song through a Vow taken by its user. Obtaining one will grant you considerable strength and a plethora of new abilities!
-   Linkstriders are peculiar folk that exist in the boundary between two states. This unique perspective grants them perception of the Bonds between all. Those who seek this Oath will need to sacrifice many to Catalyse their pursuit of Entropy...
-   Those who swear themselves to the Oathless Vow cast aside their desire for an Oath and seek their own path. These free thinkers can be found in quiet contemplation in secluded corners of the world.

### World Building

-   Some daring Pathfinders speak of the existence of mysterious shrines scattered throughout Lumen that let you exchange Knowledge to alter your character stats, talents and mantras...
-   Lawless areas have no penalty for starting fights with others, so feel free to spar in them!

* * *

## Loading Failed

When the game fails to load in properly a hidden loading screen appears.

### Stage 1

Uh oh. Where's the game?

This can't be good.

Games are supposed to _work_. Game are supposed to let you _play_ them.

...Probably won't be broken forever though, right?

Like they've probably got a guy for this, right?

Man.

### Stage 2

Dialogue Changes: Huh? Still no dice?

Maybe if we wait 30 seconds longer some suggestions for games to play while we wait will start scrolling down on the screen.

Games do that kinda thing nowadays, apparently.

As if the game breaking wasn't enough.

### Stage 3

Names of games will start to appear from the top of the screen:

-   [A Hat in Time](https://en.wikipedia.org/wiki/A_Hat_in_Time)
-   [Abyss World](https://www.roblox.com/games/12151453277/Abyss-World)
-   [Airship Assault](https://www.roblox.com/games/8840562096/Airship-Assault)
-   [Animal Crossing: New Leaf](https://en.wikipedia.org/wiki/Animal_Crossing:_New_Leaf)
-   [Baba Is You](https://en.wikipedia.org/wiki/Baba_Is_You)
-   [Backgammon](https://en.wikipedia.org/wiki/Backgammon)
-   [Bad Business](https://www.roblox.com/games/3233893879/Bad-Business)
-   [Barotrauma](<https://en.wikipedia.org/wiki/Barotrauma_(video_game)>)
-   [Battleblock Theatre](https://en.wikipedia.org/wiki/BattleBlock_Theater)
-   [Castle Crashers](https://en.wikipedia.org/wiki/Castle_Crashers)
-   [Celeste](<https://en.wikipedia.org/wiki/Celeste_(video_game)>)
-   [Checkers](https://en.wikipedia.org/wiki/Checkers)
-   [Chess](https://en.wikipedia.org/wiki/Chess)
-   [Citizen Sleeper](https://en.wikipedia.org/wiki/Citizen_Sleeper)
-   [CONE](https://www.roblox.com/games/378223328/CONE)
-   [CRAZY HORSE GAME 🐴](https://www.roblox.com/games/15716048273/CRAZY-HORSE-GAME)
-   [Crypt of the Necrodancer](https://en.wikipedia.org/wiki/Crypt_of_the_NecroDancer)
-   [Deep Rock Galactic](https://en.wikipedia.org/wiki/Deep_Rock_Galactic)
-   [Deltarune](https://en.wikipedia.org/wiki/Deltarune)
-   Don't play [ABA](https://www.roblox.com/games/1458767429/ABA) please
-   [Don't Starve Together](https://en.wikipedia.org/wiki/Don%27t_Starve_Together)
-   [DOORS](https://doors-game.fandom.com/wiki/DOORS)
-   [Dredge](<https://en.wikipedia.org/wiki/Dredge_(video_game)>)
-   [Dwarf Fortress](https://en.wikipedia.org/wiki/Dwarf_Fortress)
-   [Final Fantasy VII](https://en.wikipedia.org/wiki/Final_Fantasy_VII)
-   [Final Fantasy XIV](https://en.wikipedia.org/wiki/Final_Fantasy_XIV)
-   [Find Everything](https://www.roblox.com/games/8389627336/Find-Everything)
-   [FPS Chess](https://fps-chess.fandom.com/wiki/FPS_Chess)
-   [Goose Goose Duck](https://goose-goose-duck.fandom.com/wiki/Goose_Goose_Duck)
-   [Hades I](<https://en.wikipedia.org/wiki/Hades_(video_game)>) & [II](https://en.wikipedia.org/wiki/Hades_II)
-   [Halo 3](https://en.wikipedia.org/wiki/Halo_3)
-   [Halo 3 ODST](https://en.wikipedia.org/wiki/Halo_3:_ODST)
-   [Halo Reach](https://en.wikipedia.org/wiki/Halo:_Reach)
-   [Hello Kitty Island Adventure](https://en.wikipedia.org/wiki/Hello_Kitty_Island_Adventure)
-   [Hollow Knight](https://en.wikipedia.org/wiki/Hollow_Knight)
-   [Hollow Knight: Silksong](https://en.wikipedia.org/wiki/Hollow_Knight:_Silksong)
-   [Inscryption](https://en.wikipedia.org/wiki/Inscryption)
-   [Isle](https://www.roblox.com/games/3095204897/Isle)
-   [Kingdom](<https://en.wikipedia.org/wiki/Kingdom_(video_game)>) Series
-   [L.A. Noire](https://en.wikipedia.org/wiki/L.A._Noire)
-   [Lethal Company](https://en.wikipedia.org/wiki/Lethal_Company)
-   [Mahjong](https://en.wikipedia.org/wiki/Mahjong)
-   [Mini Metro](<https://en.wikipedia.org/wiki/Mini_Metro_(video_game)>)
-   [monkey raft](https://www.roblox.com/games/17495769916/monkey-raft)
-   [Monster Hunter](https://en.wikipedia.org/wiki/Monster_Hunter) Series
-   [Monster Hunter Wilds](https://en.wikipedia.org/wiki/Monster_Hunter_Wilds)
-   [Monster Hunter: World](https://en.wikipedia.org/wiki/Monster_Hunter:_World)
-   [My Movie](https://www.roblox.com/games/13600218266/My-Movie)
-   [Nine Sols](https://en.wikipedia.org/wiki/Nine_Sols)
-   [No Mans Sky](https://en.wikipedia.org/wiki/No_Man%27s_Sky)
-   [Noita](<https://en.wikipedia.org/wiki/Noita_(video_game)>)
-   [Outer Wilds](https://en.wikipedia.org/wiki/Outer_Wilds)
-   [Peglin](https://en.wikipedia.org/wiki/Peglin)
-   [QWOP](https://en.wikipedia.org/wiki/QWOP)
-   [R.E.P.O.](https://en.wikipedia.org/wiki/R.E.P.O.)
-   [Rainworld](https://en.wikipedia.org/wiki/Rain_World)
-   [Red Dead Redemption](https://en.wikipedia.org/wiki/Red_Dead_Redemption)
-   [Risk of Rain 2](https://en.wikipedia.org/wiki/Risk_of_Rain_2)
-   [Road to Gramby's](https://www.roblox.com/games/5796917097/Road-to-Grambys)
-   [Sekiro: Shadows Die Twice](https://en.wikipedia.org/wiki/Sekiro:_Shadows_Die_Twice)
-   [Shadow Gambit: The Cursed Crew](https://en.wikipedia.org/wiki/Shadow_Gambit:_The_Cursed_Crew) (R.I.P. [Mimimi Games](https://en.wikipedia.org/wiki/Mimimi_Games))
-   [Shadow Tactics: Blades Of The Shogun](https://en.wikipedia.org/wiki/Shadow_Tactics:_Blades_Of_The_Shogun) (R.I.P. [Mimimi Games](https://en.wikipedia.org/wiki/Mimimi_Games))
-   [Shadows of Doubt](https://en.wikipedia.org/wiki/Shadows_of_Doubt)
-   [Shovelware's Brain Game](https://www.roblox.com/games/7978512334/Shovelwares-Brain-Game)
-   [Slay the Spire](https://en.wikipedia.org/wiki/Slay_the_Spire)
-   [Snakes & Ladders](https://en.wikipedia.org/wiki/Snakes_%26_Ladders)
-   [Sonic Unleashed](https://en.wikipedia.org/wiki/Sonic_Unleashed)
-   [Space Station 14](https://store.steampowered.com/app/1255460/Space_Station_14)
-   [Stars Align](https://www.roblox.com/games/15837460390/Stars-Align-Engine-Demo)
-   [Straftat](https://en.wikipedia.org/wiki/Straftat)
-   [Subnautica](https://en.wikipedia.org/wiki/Subnautica)
-   [Sunless Skies](https://en.wikipedia.org/wiki/Sunless_Skies)
-   [Super Bomb Survival](https://www.roblox.com/games/164051105/Super-Bomb-Survival)
-   [Super Doomspire](https://www.roblox.com/games/3725149043/Super-Doomspire)
-   [Terraria](https://en.wikipedia.org/wiki/Terraria)
-   [Tetris](https://en.wikipedia.org/wiki/Tetris)
-   [The Great Ace Attorney Chronicles](https://en.wikipedia.org/wiki/The_Great_Ace_Attorney_Chronicles)
-   [Townscaper](https://en.wikipedia.org/wiki/Townscaper)
-   [Tricky Towers](https://en.wikipedia.org/wiki/Tricky_Towers)
-   [Twilight Imperium 4](https://en.wikipedia.org/wiki/Twilight_Imperium_4)
-   [Undertale](https://en.wikipedia.org/wiki/Undertale)
-   [V Rising](https://en.wikipedia.org/wiki/V_Rising)
-   [Valheim](https://en.wikipedia.org/wiki/Valheim)
-   [WEBFISHING](https://en.wikipedia.org/wiki/WEBFISHING)
-   [Wobbledogs](https://en.wikipedia.org/wiki/Wobbledogs)
