---
title: The Endless Wave
revid: 203860
source: https://deepwoken.fandom.com/wiki/The_Endless_Wave
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Exclusive, Weapons, Greatswords, Weapons with Unique Criticals, Weapons with Unique Animations, Heavy Weapons, Weapons with multiple Criticals, "Weapons with Multi-Hit Criticals"]
notices:
  - "This page describes Exclusives/Specs This content is currently exclusive to Developers or other notable members, and as such, is not available to the general public."
---

# The Endless Wave

| The Endless Wave | |
|---|---|
| type | Greatsword |
| damage type | Slash |
| requirements | 60 HVY, 50 STR |
| damage | 25 |
| scaling | HVY: 8, STR: 1 |
| posture damage | 10 |
| range | 15 Sweep |
| swing speed | 0.92x |
| attack duration | 0.7s |
| penetration | 50% |
| m.equip | No |
| m.extract | No |
| stances | 2 Handed, 1 Handed (≥40 HVY) |
| rarity | Mythical |
| selling price | 100 |
| weight | 2 |
| bankable | No |
| droppable | No |
| enchantable | Yes |
| pass down | No |
| obtainment | Unobtainable |
| description | The largest set of odachi's ever manufactured by Etrea in it's prime, not a lot in this world can hold a candle to the craftsmanship of this behemoth. |

## Description

The Endless Wave is a Greatsword [Heavy Weapon](Weapons.md). Equipping this requires 60 HVY and 50 STR invested into your character.

## Critical

For the **standing** critical, the user thrusts their weapon forwards, dealing damage and ragdolling enemies. Deals **1.45x scaled damage**.

For the **sliding** critical, the user sweeps their weapon in a 360° angle, dealing damage to those caught in it. The user can follow-up by clicking M1 while casting (and successfully landing) the sliding critical which will cause the user to perform the standard critical immediately afterwards. Do note that enemies will recover in time to parry the follow-up crit. The initial slash of the sliding critical deals **1.25x scaled damage**, and the M1 follow-up deals the same amount of damage as the normal critical **(1.45x scaled damage)**.

The cooldown for either critical is **10 seconds**.

## Trivia

-   This weapon is wielded by the [Shogun of the Prophet's Guard](<Shogun_of_the_Prophet's_Guard.md>) found in the [Vow of Iron](Vow_of_Iron.md) gamemode.
