---
title: Authority Cager
revid: 209945
source: https://deepwoken.fandom.com/wiki/Authority_Cager
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Vow of Iron Exclusive, Enemies, Central Authority Enemies, Enemies that can wield Enchantments]
---

# Authority Cager

| Authority Cager | |
|---|---|
| health | ??? |
| weapon | Duskguard Axe |
| moves | Assault, Critical, Storm Blades |

**[Vow of Iron](Vow_of_Iron.md) Exclusive**  
This page describes content that is exclusive to the **Vow of Iron** gamemode.  
This content is not accessible outside of this gamemode.

## Description

Authority Cagers are a type of enemy NPC that are operators for the [The Central Authority](The_Central_Authority.md) that are found near a bunch of Explosive Crates with an Authority [Thresher](Thresher.md) by its side. They can spawn in the following places:

-   Encountered in the Cargo Recovery [job](Job_Boards.md).
-   [Miner's Landing (Vow of Iron)](<Miner's_Landing_(Vow_of_Iron).md>) near the dock.
-   The [Summer Isle](Summer_Isle.md) Castle Ruins.

Authority Cagers wear a black [Shrouded Cape](Shrouded_Cape.md), an [Authority Helm](Authority_Helm.md), a pair of [Bluesteel Pauldrons](Bluesteel_Pauldrons.md), a pair of [Bluesteel Boots](Bluesteel_Boots.md) and an [Shock Corps Light](Shock_Corps_Light.md).

## Moveset

Their arsenal consists of a [Duskguard Axe](Duskguard_Axe.md) (can be enchanted) and the [Thundercall](Thundercall.md) Attunement:

| Name/Danger | Description |
| --- | --- |
| 
**Assault**  
(★☆☆)

 | Performs 3 basic attacks before sending the opponent away with a flourish.

 |
| 

**Critical**  
(★★☆)

 | Raises axe upward before slamming it down to the ground.

 |
| 

**Storm Blades (Tornado Spark)**  
(★☆☆)

 | Authority Cager conjures a ring of lightning blades.

 |

## Finisher

An Authority Cager executes the player after knocking them.

| Name | Description |
| --- | --- |
| Execute | Splits the enemy's head in two with their Axe. |
