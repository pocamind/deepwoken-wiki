---
title: Brilliant Cape
revid: 175214
source: https://deepwoken.fandom.com/wiki/Brilliant_Cape
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Equipment, Torso, Capes]
---

# Brilliant Cape

| Brilliant Cape | |
|---|---|
| type | Torso |
| rarity | Uncommon |
| selling price | 14 |
| requirements | PWR 5 |
| weight | 0.5 |
| innate stats | 1 Legendary Pip3% Elemental Armor |
| innate talent | N/A |
| set | Brilliant Pauldron, Brilliant Boots, Brilliant Cape |
| set talent | N/A |
| bankable | Yes |
| droppable | Yes |
| enchantable | No |
| pass down | Yes |
| obtainment | Other sources:, Dread Serpent drop, The Diluvian Mechanism, Sold in Traan Zakshun's black market for 1 |
| description | ... |

## Description

The Brilliant Cape is an animated [Equipment](Equipment.md) item under the "Torso" category, exclusively obtained as a drop from the [Dread Serpent](Dread_Serpent.md) and Wave 50 chests from [The Diluvian Mechanism](The_Diluvian_Mechanism.md).

## Trivia

-   The cape, along with the entire Brilliant set Is a reference to [Ike](https://fireemblemwiki.org/wiki/Ike) from his appearance in [Fire Emblem: Radiant Dawn](https://fireemblemwiki.org/wiki/Fire_Emblem:_Radiant_Dawn).
