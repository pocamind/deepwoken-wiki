---
title: Cathedral of the Interstice
revid: 126196
source: https://deepwoken.fandom.com/wiki/Cathedral_of_the_Interstice
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Locations, Lost Celtor]
---

# Cathedral of the Interstice

LocationL Cathedral of Insterstice LocationR

TERRITORY OF Lost Celtor (Lawless Territory)

  

The **Cathedral of the Interstice** is a sub-location in [The Depths](The_Depths.md). It is the main way for a player to escape the Depths through the Depths Trial. It contains the [Shrine of Courage](Shrine_of_Courage.md) that heavily increases the difficulty of the trial, often adding a [Bone Keeper](Bone_Keeper.md) onto what you normally need to fight but giving you enough experience to invest 15 stat points. There are few ways to enter this area, the first being a gate in the District of Commerce in the [The City of the Drowned](The_City_of_the_Drowned.md) that is opened after ringing 2 bells located at the district. The second is using an elevator located in The City of the Drowned on the opposite side of the District of Commerce, (Its usage costs 25 notes). There are other more obscure methods, such as using the [Dimensional Travel Resonance](Dimensional_Travel.md).

  
Once you use the lift to exit the depths, you may face certain enemies. The enemies are chosen by a hidden stat called **Trial Power**. Your Trial Power equals **your current Power + (Times drowned \* 0.33)**. If you have the following Trial Power, you will face these enemies:

| _Trial_ Power | Enemy(s) |
| --- | --- |
| 1-3 | Nothing (you are sent up without a fight) |
| 3.33-6.33 | [Mudskipper Knight](Mudskipper_Knight.md) |
| 6.66-9.66 | [Angel](Hollow_Angel.md) (Lesser) |
| 10-16.33 | [Megalodaunt](Megalodaunt.md) |
| 16.66+ | [Enforcer](Enforcer.md) |
| 20 | If you have over 20 drowns, have the Vow of Thorns echo modifier, or have activated the [Shrine of Courage](Shrine_of_Courage.md), then waves of enemies will spawn, each consisting of two Monsters, which can be any combination of [Bone Keeper](Bone_Keeper.md), [King Thresher](Thresher.md), both of which can spawn corrupted, or [Fury Nautilodaunt](Nautilodaunt.md) |
| 5+ people (must all be power 20) | Initiates a fight with the [Heart of Enmity](Heart_of_Enmity.md). |

These values are not set in stone, as there have been times where characters at Power 5 have escaped without a fight.

For more information on the trial, go to this page: [First Layer](First_Layer.md).
