---
title: Keyblade
revid: 153179
source: https://deepwoken.fandom.com/wiki/Keyblade
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Exclusive, Weapons, Developer Spec]
notices:
  - "This page describes Exclusives/Specs This content is currently exclusive to Developers or other notable members, and as such, is not available to the general public."
---

# Keyblade

| Keyblade | |
|---|---|
| type | Elemental Weapon (Unconfirmed) |
| requirements | (Unknown) |
| damage | ??? |
| scaling | ??? |
| range | ??? |
| swing speed | ??? |
| endlag | ??? |
| penetration | ??? |
| chip damage | ??? |
| special effect | Hero Blade of Wind Critical |
| selling price | ??? |
| weight | ??? |
| bankable | Yes |
| droppable | Yes |
| enchantable | Yes |
| pass down | Yes |
| obtainment | Unobtainable |
| description | ??? |

## Description

The Keyblade is a developer spec which is assumed to be a reskin to the [Hero Blade of Wind](Hero_Blades.md) if this is the case it has the exact stats of the Hero Blade of Wind, Otherwise it's critical might’ve been taken from the Hero Blade and applied to this weapon.

## Trivia

-   It's more than likely inspired by the Keyblade from [Kingdom Hearts](https://kingdomhearts.fandom.com/wiki/Keyblade%7C).
-   This skin belongs to the moderator "Sora".
