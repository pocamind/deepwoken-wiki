---
title: Fragments of Self
revid: 23827
source: https://deepwoken.fandom.com/wiki/Fragments_of_Self
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Pages with broken file links, Locations, Death]
---

# Fragments of Self

LocationL Fragments of Self LocationR

What will become of us?

  

File:FragmentsofSelfThumbnail.png

**Fragments of Self** is a location in _Deepwoken_. It is a sub-location in [The Depths](The_Depths.md).

## Overview

Fragment of Self acts as the purgatory of _Deepwoken_. Players are sent to it whenever they die in [The Depths](The_Depths.md).

## Contents

#### Campfire

-   Players can use this to remove Equipment they want to pass down at the Hippocampal Pool.

#### Hippocampal Pool

-   Located to the left, interacting with this allows you to pass down two items that will transfer to your next character.

#### Shrine of the Drowned

-   Located before Self, a statue that allows you to pay 1,000 to return to the Surface. This can only be used once per character and requires [Power](Power.md) 10.

#### Self

-   Located past the stone arch, a pure reflection of yourself that will remove your character from _Deepwoken_.
    -   Clicking \[The End\] will wipe your character and transfer you to the character creation screen.

## Trivia/Notes

-   If you fell off the Fragment of Self floating island, you will be teleported back to spawn.
-   There's is currently no known way to die in Fragments of self, as fall damage is disabled and so is any form (Potions?) of PVP damage, starving also only drains the player's health down to 50%, it is unclear if bleeding out is possible, and dying here would most definitely be a bug and would respawn you in Fragments of Self again.
