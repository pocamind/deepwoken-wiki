---
title: Wrath Gem
revid: 176910
source: https://deepwoken.fandom.com/wiki/Wrath_Gem
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Treasures, Mantras, Deep Gems]
---

# Wrath Gem

| Wrath Gem | |
|---|---|
| value | 200 |
| weight | 1 |
| stack | x25 |
| type | Deep Gem |
| bankable | Yes |
| droppable | Yes |
| pass down | No |
| desc | It hurts to hold...On hit, apply Wrath. |
| obtainment | Titus Fulminus Destellus |

## Description

Wrath Gem is an light red diamond-shaped [Deep Gem](Deep_Gems.md).

## Effect

Upon landing a Mantra with a Wrath Gem equipped, apply the [Wrath](Status_Effects.md#wrath) status effect for 9 seconds, increasing your critical attack damage by 7.5% per player combat tag you have. This effect has a 10 second cooldown.

### Notes

-   If you are using the [Reinforced War Set](Reinforced_War_Helmet.md), then the Wrath Gem is best used on [Strength Mantras](Attunement-less.md#strength-mantras), to proc their equipment Talents. If not, then this Gem will be most effective on any easy-to-hit Mantra with a low cooldown.
    -   Additionally, using a weapon with a high damage critical is highly recommended, as it will benefit more from the percentage damage increase. If you are using the Reinforced War Set, then it is recommended to use a weapon that has a low critical cooldown or a critical that resets its critical cooldown on hit, as two of the equipment Talents proc on critical hit.
-   If you have the Brain [Food Buff](Status_Effects.md#food-buff) from the [Grilled Parasol Brain](Grilled_Parasol_Brain.md), Wrath Gem's damage buff per player combat tag will be increased from 7.5% to 15%.
-   As previously mentioned, the Wrath status effect can gain additional effects through the equipment Talents in the [Reinforced War Set](Reinforced_War_Helmet.md).
    -   The [Fatal Strike](Talents.md#fatal-strike) Talent from the [Reinforced War Helmet](Reinforced_War_Helmet.md) increases the damage of your [Strength Mantra](Attunement-less.md#strength-mantras) guardbreaks while they have Wrath Gems equipped.
    -   The [Crippling Impact](Talents.md#crippling-impact) Talent from the [Reinforced War Plate](Reinforced_War_Plate.md) disables your opponent's [Vent](Combat_Mechanics.md#tempo) for a few seconds if you land a critical attack while your opponent is afflicted with the [Wrath](Status_Effects.md#wrath) status effect. The duration of this effect scales on your [Strength](Attributes.md#str) investment.
    -   The [Repeated Blows](Talents.md#repeated-blows) Talent from the [Reinforced War Boots](Reinforced_War_Boots.md) resets the cooldown of your [Strength Mantras](Attunement-less.md#strength-mantras) if you land a critical attack while your opponent is afflicted with the [Wrath](Status_Effects.md#wrath) status effect.
    -   The [Lasting Trauma](Talents.md#lasting-trauma) Set Talent from the Reinforced War Set additionally applies the [Fearful](Status_Effects.md#fearful) status effect for 9 seconds whenever you land a Mantra with a Wrath Gem equipped. The Fearful status effect disables your opponent's ability to use their critical attack.
-   For usage with [Critical Engine](Talents.md#critical-engine), the damage buff will not be applied to the hit that procced the Deep Gem, but will be applied to subsequent hits after granting the Wrath status effect. It is highly suggested that you use a multi-hitting critical for this Gem.
-   Wrath Gem's damage buff is affected by the damage modifier cap.

## Crafting

This Gem can also be used to craft a variety of items.

-   [Equipment](Equipment.md): [Reinforced War Helmet](Reinforced_War_Helmet.md), [Reinforced War Plate](Reinforced_War_Plate.md), and [Reinforced War Boots](Reinforced_War_Boots.md).
