---
title: "Adventurer's Scarf"
revid: 211080
source: https://deepwoken.fandom.com/wiki/Adventurer's_Scarf
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Equipment, Torso, Scarves]
---

# Adventurer's Scarf

| Adventurer's Scarf | |
|---|---|
| type | Torso |
| rarity | Variant Based |
| selling price | Variant Based |
| requirements | Power 5 |
| weight | 0.5 |
| innate stats | +3% Attunement Armor, depending on variant, +1 Rare Pip, +1 Legendary Pip |
| innate talent | N/A |
| bankable | Yes |
| droppable | Yes |
| enchantable | No |
| pass down | Yes |
| obtainment | Aratel Island, Etris: Siege (Black/White), Starswept Valley, Aelita (Black), The Ferryman (Only in Winter) (Black), Saramaed Hollow |
| description | ... |

## Description

Scarves are an [Equipment](Equipment.md) item under the "Torso" category. They can be found through chests on [Aratel Island](Aratel_Island.md).

## Notes

<table><caption></caption>
<tbody>
<tr>
  <th>Variant</th>
  <th>Rarity</th>
  <th>Protection</th>
  <th>Price</th>
</tr>
<tr>
  <td>Adventurer's</td>
  <td>Rare</td>
  <td>3% Wind Armor <a href="Galebreathe.md">ResistanceGalebreathe</a></td>
  <td>25</td>
</tr>
<tr>
  <td>Black</td>
  <td>Uncommon</td>
  <td>3% Shadow Armor <a href="Shadowcast.md">ResistanceShadowcast</a></td>
  <td>14</td>
</tr>
<tr>
  <td>Crimson</td>
  <td>Common</td>
  <td>3% Fire Armor <a href="Flamecharm.md">ResistanceFlamecharm</a></td>
  <td>7</td>
</tr>
<tr>
  <td>Dawn</td>
  <td>Rare</td>
  <td>3% Shadow Armor <a href="Shadowcast.md">ResistanceShadowcast</a></td>
  <td>25</td>
</tr>
<tr>
  <td>Desert</td>
  <td>Uncommon</td>
  <td>3% Fire Armor <a href="Flamecharm.md">ResistanceFlamecharm</a></td>
  <td>14</td>
</tr>
<tr>
  <td>Forest</td>
  <td>Rare</td>
  <td>3% Lightning Armor <a href="Thundercall.md">ResistanceThundercall</a></td>
  <td>25</td>
</tr>
<tr>
  <td>Tundra</td>
  <td>Uncommon</td>
  <td>3% Ice Armor <a href="Frostdraw.md">ResistanceFrostdraw</a></td>
  <td>14</td>
</tr>
<tr>
  <td>White</td>
  <td>Uncommon</td>
  <td>3% Shadow Armor <a href="Shadowcast.md">ResistanceShadowcast</a></td>
  <td>14</td>
</tr>
</tbody>
</table>

**All scarves share the same pips.**

## Trivia

-   The scarves have the highest variety of any equipment, having a total of 8 variants.
