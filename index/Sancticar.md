---
title: Sancticar
revid: 210375
source: https://deepwoken.fandom.com/wiki/Sancticar
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Weapons, Criticals with M1 tag, "Weapons with Multi-Hit Criticals", Staves, Medium Weapons, Weapons with Talents]
---

# Sancticar

| Sancticar | |
|---|---|
| type | Staff |
| damage type | Blunt |
| requirements | 50 MED, Origin: Justicar |
| damage | 19.5 |
| scaling | MED: 6.5 |
| posture damage | 6 |
| range | 8 Sweep |
| swing speed | 0.92x |
| attack duration | 0.7s |
| penetration | 15% |
| talent | Both Ends - Pressing M2 after landing a light attack with the staff performs a follow-up swing from the opposite end. |
| stances | 2 Handed, 1 Handed |
| rarity | Mythical |
| selling price | 100 |
| weight | 2 |
| bankable | No |
| droppable | No |
| enchantable | Yes |
| pass down | No |
| obtainment | Sold at Alsin's Landing for 100 . |
| description | ... |

## Description

The Sancticar is a [Staff](Weapons.md) weapon in the Medium category.

## Obtainment

The Sancticar spawns behind a white tree in the northeastern room of [Alsin's Landing](<Alsin's_Landing.md>), where it can be brought up by the player for 100.

Location

## Talent: Both Ends

Equipping the Sancticar grants its unique [weapon Talent](Talents.md#weapon-talents)(s). Weapon Talents will be lost upon equipping any other weapon, including ability-bound weapons, like from the [Crazy Slots](Crazy_Slots.md) Resonance.

**Both Ends** allows you to followup an M1 with a swing from the opposite end of the staff by pressing M2. This attack comes out much faster, but only does 45% of your weapon damage. This has the M1 tag, allowing it to proc M1-exclusive [Talents](Talents.md) and [Enchantment](Enchantments.md) effects.

## Critical

Sancticar uses the default Staff critical, a pole vault before slamming the staff onto the target, knocking them away and applying [Knockdown](Status_Effects.md#knockdown). It has a scaled damage multiplier of **0.4x** on the pole vault and **2.2x** on the slam, and has a cooldown of **6 seconds**. This critical can break destructible terrain.

This critical has the M1 tag, allowing it to proc M1-exclusive [Talents](Talents.md) and [Enchantment](Enchantments.md) effects.
