---
title: Kyrstear
revid: 98949
source: https://deepwoken.fandom.com/wiki/Kyrstear
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Weapons, Verse 2, Parrying Daggers]
---

# Kyrstear

| Kyrstear | |
|---|---|
| type | Offhand Weapon |
| requirements | 40 AGL |
| damage | 5 |
| scaling | 8 |
| range | 6.5 |
| swing speed | 0.98x |
| endlag | 0.05s |
| penetration | N/A |
| chip damage | N/A |
| special effect | Decreases your posture gain on parry. Increases your opponents posture gain when parried. |
| offhand | Offhand only |
| weight | 4 |
| bankable | Yes |
| droppable | Yes |
| enchantable | No |
| pass down | Yes |
| obtainment | Layer 2 Chests |
| description | "Your reliable offhand." |

## Description

Kyrstear is an offhand weapon added in the [Verse 2](Verse_2.md) update. 40 agility is required to wield it.

Its function is the same as that of the [Parrying Dagger](Parrying_Dagger.md), allowing the user to recover ??% (more than parrying dagger) more posture from parries.

## Trivia

-   A part of the Kyrs series ([Kyrsedge](Kyrsedge.md), **Kyrstear**, [Kyrstreza](Kyrstreza.md), [Kyrsblade](Kyrsblade.md), [Kyrsglaive](Kyrsglaive.md), [Kyrscleave](Kyrscleave.md))
-   One of two Parry Dagger type weapons.
-   Seems to be paired with Kyrsedge.
-   The weapon used to have a bug where the agility requirement did not apply. This has been fixed.
