---
title: "Light's Final Toll"
revid: 47425
source: https://deepwoken.fandom.com/wiki/Light's_Final_Toll
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Weapons, Pages with broken file links, Fists, "Core-Attribute based weapons"]
---

# Light's Final Toll

| Light's Final Toll | |
|---|---|
| type | Fist |
| requirements | 65 WLL & Fist Style |
| damage | 12 (27.9) |
| scaling | 13 WLL |
| range | 7 |
| swing speed | 0.87x |
| penetration | 5% |
| special effect | Successfully flourishing an opponent rings the lantern, reducing sanity of the affected. |
| stances | 1 Handed |
| weight | 5 |
| bankable | Yes |
| droppable | Yes |
| enchantable | Yes |
| pass down | Yes |
| obtainment | Layer 2 |
| description | A reminder that all songs must come to an end. |

## Description

Light's Final Toll is a legendary blue spirit lantern that floats beside the player. The player also wears a string of prayer beads around their arm. It can be obtained from [Layer 2](Second_Layer.md). It is the only current weapon to scale off of Willpower.

## Trivia

-   This is the first weapon to scale from an attribute other than weapon or an attunement.
-   This comes from SudoMesh's entry (Light's Final Toll) from the Discord's art contest to have their weapon concept added into the game.
    -   As the winner of the contest, he was also given the exclusive [Aberrant Capra](Aberrant_Capra.md) subrace.
-   The only weapon that can gain scaling past 100, as with +2 WLP races, you can get to a "weapon stat" of 102.
-   **Is not compatible with Jus Karita.**

File:Lantern.png

Original Concept Art
