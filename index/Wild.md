---
title: Wild
revid: 217979
source: https://deepwoken.fandom.com/wiki/Wild
license: CC BY-SA 3.0 (content derived from the Deepwoken Wiki)
categories: [Enchantments, Weapon Enchantments]
---

# Wild

| Wild | |
|---|---|
| bankable | Yes |
| droppable | No |
| pass down | Yes |
| desc | On hit, deal 85% of your weapon's damage in an AoE. This AoE cannot hit allies or the enemy originally hit. Reduces your damage against allies by 50%. |
| obtainment | Finding a Wild enchanted weapon in a chest, LaplaceWild Enchant Stone:, Exchanging 5 to Klaris (grants a random non-exclusive Enchant Stone), War Mode, Hell Mode, Scion of Ethiron, Maestro Evengarde Rest, Dread Serpent, Heart of Enmity, Chaser, Scholar of the Crimson Contract, Saramaed Hollow, Second Layer, Diluvian Mechanism, Galewax Room in the Second Layer, Crafting :, Any Enchant Stone, Cloudstone, Wind Gem |

## Description

Wild is a damage dealing [enchantment](Enchantment.md) that applies deals your weapon's damage in an AoE.

## Effect

On weapon hit, Wild deals 85% of your weapon's damage in an ~18.25 stud AoE around the target you hit. This cannot hit the target who was hit by the initial attack, unless two targets were hit at the same time.

Allies take 50% of your weapon's damage from Wild instead of the full 85%.

### Notes

-   Wild's damage is dealt as typeless damage, bypassing armor resistances.
-   Wild's AoE can only proc once per weapon attack; even if you hit multiple enemies with the same attack, Wild will only proc once.
